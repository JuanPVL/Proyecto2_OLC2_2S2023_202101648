	...
	case "+":
		{
			...
			if dominante == environment.INTEGER || dominante == environment.FLOAT {
				...
				gen.AddExpression("P", "P", "1", "+")
				newTemp2 := gen.NewTemp()
				gen.AddSetStack("(int)"+newTemp2, op1.Value)
				newTemp3 := gen.NewTemp()
				gen.AddExpression("P", "P", "2", "+")
				gen.AddSetStack("(int)"+newTemp3, op2.Value)
				gen.AddCall("swift_concatString")
				gen.AddGetStack(newTemp1, "(int)P")
				gen.AddExpression("P", "P", "1", "-")
				gen.AddBr()
				result = environment.NewValue(newTemp1, true, dominante, false, false, false)
				return result
			} else {
